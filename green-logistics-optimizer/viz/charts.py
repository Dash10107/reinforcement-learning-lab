...
def city_map(env: GreenCityEnv, results: list[RouteResult]) -> PIL.Image.Image:
    ...
    for r in results:
        ...
        if len(r.path) > 1:
            ...
            ax.plot(
                ...
                label=f"{r.agent_name} ({len(r.path) - 1}steps)",
            )

    # Start & goal
    start = results[0].path[0] if results else [0, 0]
    ax.scatter(
        start[1], start[0], s=200, marker="s", color=BLUE, zorder=7, linewidths=0
    )
    ax.text(
        start[1],
        start[0],
    ...
    ax.set_title(
        "City Carbon Map  ·  All Agent Routes",
        color=TEXT,
        fontsize=11,
        pad=10,
        fontfamily="monospace",
    ...
    legend_items = [
        mpatches.Patch(color=AGENT_COLORS.get(r.agent_name, TEXT), label=r.agent_name)
        for r in results
    ]
    legend_items += [
...
def carbon_trace(results: list[RouteResult]) -> PIL.Image.Image:
    ...
    bars = ax2.bar(
        names, totals, color=cols, edgecolor=BG, linewidth=0.5, alpha=0.85, width=0.5
    )
...
def performance_radar(results: list[RouteResult], env_size: int) -> PIL.Image.Image:
    ...
    fig, axes = plt.subplots(
        1, ncols, figsize=(4 * ncols, 4), subplot_kw={"polar": True}, facecolor=BG
    )
    if ncols == 1:
        axes = [axes]
    fig.patch.set_facecolor(BG)
    ...
    for ax, r in zip(axes, results[:ncols]):
        ...
        ax.set_thetagrids(
            [a * 180 / np.pi for a in angles], labels, fontsize=8, color=TEXT
        )
        ax.set_ylim(0, 10)
    ...
    fig.suptitle(
        "PERFORMANCE RADAR  ·  All Strategies",
        color=TEXT,
        fontsize=10,
        fontfamily="monospace",
        fontweight="bold",
...
def training_curve(state) -> PIL.Image.Image:
    ...
    info = [
        ("Progress", f"{pct:.1f}%"),
        ("Steps", f"{state.timestep:,}"),
        ("Episodes", str(n_ep)),
        ("Rolling Reward", f"{roll:.2f}"),
    ]
