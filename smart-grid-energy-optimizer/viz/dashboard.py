...
def _set_hour_ticks(ax, step=4):
    ...
    ax.set_xticklabels([H_LABELS[i] for i in range(0, 24, step)],
                       color=DIM, fontsize=7, rotation=30)


def dispatch_dashboard(log: dict, scenario_name: str = "", strategy: str = "") -> Figure:
    """
    ...
    _style(ax1, "⚡ ELECTRICITY PRICE  &  BATTERY STATE OF CHARGE", ylabel="Price (¢/kWh)")
    ax1b = ax1.twinx()
    ...
    ax1b.plot(hours, [s / max(log["soc"] + [1e-9]) * 100 for s in log["soc"]],
              color=C_SOC, linewidth=2.5, linestyle="--", label="SOC (%)")
    ...
    ax1.legend(lines1 + lines2, labels1 + labels2,
               facecolor=BG3, edgecolor=GRID, labelcolor=TEXT, fontsize=8,
               loc="upper left")
    ...
    grid_export = np.array(log["grid_export"])

    ax2.fill_between(hours, 0, solar, alpha=0.7, color=C_SOLAR, label="Solar generation")
    ax2.fill_between(hours, 0, grid_import, alpha=0.5, color=C_GRID_I, label="Grid import")
    ax2.fill_between(hours, 0, -charge, alpha=0.6, color=C_BAT_C, label="Battery charging")
    ax2.fill_between(hours, 0, discharge, alpha=0.6, color=C_BAT_D, label="Battery discharge")
    ax2.plot(hours, load, color=C_LOAD, linewidth=2.5, linestyle="--",
             label="Building load", zorder=5)
    ...
    bat_cycles = sum(abs(np.array(log["action_kw"]))) / 2 / max(max(log["soc"] + [1e-9]), 1)
    peak_import = max(grid_import) if any(grid_import) else 0

    kpis = [
        ("Daily Net Revenue", f"${total_r:+.3f}",
         C_BAT_C if total_r >= 0 else C_BAT_D),
...
def comparison_chart(results: dict[str, dict]) -> Figure:
    ...
    colors = {"DP Optimal": C_DP, "DQN Agent": C_DQN,
              "Rule-Based": C_NAIVE, "No Storage": C_NOBAT}

    fig = plt.figure(figsize=(14, 8), facecolor=BG)
    gs = gridspec.GridSpec(2, 2, figure=fig, hspace=0.5, wspace=0.32,
    ...
    for name, data in results.items():
        ...
        if log and log["soc"]:
            ...
            ax1.plot(log["hour"], log["soc"], color=col, linewidth=2,
    ...
    bars = ax2.bar(strategies, rewards, color=cols,
    ...
    for name, data in results.items():
        ...
        if log and log["grid_import"]:
            col = colors.get(name, TEXT)
            ax3.plot(log["hour"], log["grid_import"], color=col, linewidth=1.8,
...
def training_chart(state) -> Figure:
    ...
    if state.episode_rewards:
        ...
        ax1.axhline(state.best_reward, color=C_SOLAR, linestyle="--",
                    linewidth=1, label=f"Best: ${state.best_reward:+.3f}")
        ax1.legend(facecolor=BG3, edgecolor=GRID, labelcolor=TEXT, fontsize=8)
    ...
    for i, (label, val) in enumerate(info):
        ...
        ax3.text(0.65, y, val, transform=ax3.transAxes,
                 color=C_SOC, fontsize=9, fontweight="bold", fontfamily="monospace")

    fig.suptitle(
        f"DQN TRAINING  ·  {state.timestep:,}/{state.total_timesteps:,} steps",
        color=TEXT, fontsize=10, fontfamily="monospace", fontweight="bold",
    )
    return fig


def scenario_price_chart(prices: np.ndarray, solar: np.ndarray,
                         load: np.ndarray, scenario_name: str = "") -> Figure:
    ...
    fig.suptitle(f"SCENARIO PREVIEW  ·  {scenario_name}",
