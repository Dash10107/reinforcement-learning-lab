...
def _draw_maze_frame(
    ...
    if walked:
        ...
        ax.text(
            ac, ar, "●", ha="center", va="center", color="white", fontsize=8, zorder=6
        )
...
def make_training_chart(
    ...
    for name, rewards in rewards_dict.items():
        ...
        if len(eps) > 20:
            ...
            ax.plot(
                range(k - 1, len(eps)), smooth, color=col, linewidth=2.2, label=name
            )
        else:
    ...
    ax.set_title(
        "Training Progress — Reward per Episode",
        color=TEXT_C,
        fontsize=11,
        pad=10,
        fontfamily="monospace",
...
def make_qvalue_heatmap(
    env: MazeEnv, agent: TabularAgent, title: str = ""
) -> MplFigure:
    ...
    ax2.set_title(
        "Q-Value Map (brighter = agent prefers this cell)",
        color=TEXT_C,
        fontsize=10,
        pad=8,
        fontfamily="monospace",
...
def make_race_chart(
    ...
    name_b: str,
    rewards_c: list[float] | None = None,
    name_c: str = "",
) -> MplFigure:
    ...
    ax.set_title(
        "Reward Convergence", color=TEXT_C, fontsize=10, fontfamily="monospace"
    )
