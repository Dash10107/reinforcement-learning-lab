...
from warehouse.layout import DELIVERY, PICKUP, SHELF
...
def render_frame(
    ...
    for i, robot in enumerate(robots):
        col = ROBOT_COLORS[i % len(ROBOT_COLORS)]
        ax.plot(
            [robot.col, robot.goal_col],
            [robot.row, robot.goal_row],
            color=col,
            linewidth=0.7,
            alpha=0.35,
            linestyle=":",
    ...
    for i, robot in enumerate(robots):
        ...
        ax.text(
            ...
            ha="center",
            va="center",
            color="white" if not robot.has_package else col,
            fontsize=5,
            fontweight="bold",
            zorder=7,
        )
    ...
    fig.savefig(buf, format="png", dpi=130, bbox_inches="tight", facecolor=C_BG)
    buf.seek(0)
    plt.close(fig)
