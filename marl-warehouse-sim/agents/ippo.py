...
class RobotAgent:
    ...
    def store(self, obs, action, lp, reward, done, value):
        ...
        self.lp_buf.append(lp);   self.rew_buf.append(reward)
        self.done_buf.append(done); self.val_buf.append(value)

    def update(self, last_obs: np.ndarray,
               gamma: float = 0.95, lam: float = 0.95,
               clip: float = 0.2, n_epochs: int = 4,
               batch_size: int = 64, ent_coef: float = 0.01) -> dict:
        ...
        for t in reversed(range(len(self.rew_buf))):
            delta = self.rew_buf[t] + gamma * vals[t+1] * (1 - self.done_buf[t]) - vals[t]
        ...
        for _ in range(n_epochs):
            ...
            for s in range(0, len(idx), batch_size):
                ...
                self.actor_opt.step()
                self.critic_opt.zero_grad(); cl.backward()
                nn.utils.clip_grad_norm_(self.critic.parameters(), 0.5)
                self.critic_opt.step()

                al_total += al.item(); cl_total += cl.item(); n += 1

        self.obs_buf.clear(); self.act_buf.clear(); self.lp_buf.clear()
...
class WarehouseIPPO:
    ...
    def act_all(self, obs_dict: dict) -> tuple[dict, dict, dict]:
        ...
        for aid, agent in self.agents.items():
            ...
            actions[aid] = a; lps[aid] = lp; vals[aid] = v
        return actions, lps, vals

    def greedy_all(self, obs_dict: dict) -> dict:
        return {aid: agent.actor.greedy(obs_dict[aid]) for aid, agent in self.agents.items()}

    def store_all(self, obs, actions, lps, rewards, dones, vals):
        for aid, agent in self.agents.items():
            agent.store(obs[aid], actions[aid], lps[aid],
                        rewards[aid], dones[aid], vals[aid])
    ...
    def save(self, path: str = CHECKPOINT):
        ...
        } for aid, a in self.agents.items()}, path)
