...
from core.environment import SUBJECTS, SUBJECT_COLORS
...
def trajectory_chart(history: list[dict]) -> PIL.Image.Image:
    ...
    _style(ax1, "LEARNING TRAJECTORY  —  Proficiency Over Steps",
           "", "Proficiency (%)")
    ...
    ax1.axhline(98, color=TEXT, linewidth=0.7, linestyle="--", alpha=0.3,
                label="Mastery (98%)")
    ax1.set_ylim(-2, 105)
    ax1.legend(facecolor=BG2, edgecolor=GRID, labelcolor=TEXT, fontsize=7,
    ...
    for step, action in zip(steps, actions):
        ax2.bar(step, 1, color=SUBJECT_COLORS[action], edgecolor=BG,
                linewidth=0.3, alpha=0.85, width=0.9)

    ax2.set_xlim(steps[0] - 0.5, steps[-1] + 0.5)
    ax2.set_yticks([])
    ax2.set_xlabel("Step", color=DIM, fontsize=8)
    ax2.set_title("AGENT FOCUS  (colour = subject chosen)",
                  color=TEXT, fontsize=8, pad=5,
                  fontfamily="monospace", fontweight="bold")
...
def policy_bars(probs: list[float], current_state: list[float]) -> PIL.Image.Image:
    ...
    # Proficiency
    bars1 = ax1.barh(y_pos, current_state, color=SUBJECT_COLORS,
    ...
    bar_colors = [SUBJECT_COLORS[i] if i == best else "#334155"
                  for i in range(len(SUBJECTS))]
    bars2 = ax2.barh(y_pos, [p * 100 for p in probs],
    ...
    for bar, val in zip(bars2, probs):
        ax2.text(val * 100 + 1, bar.get_y() + bar.get_height()/2,
                 f"{val*100:.1f}%", va="center", color=TEXT, fontsize=8)
...
def episode_analytics(history: list[dict]) -> PIL.Image.Image:
    ...
    wedges, texts, autotexts = ax1.pie(
        ...
        colors=SUBJECT_COLORS, autopct="",
        startangle=90, wedgeprops={"edgecolor": BG, "linewidth": 2},
    )
    for t in texts:
        t.set_color(TEXT); t.set_fontsize(7.5)
    ax1.set_title("AGENT ATTENTION ALLOCATION",
    ...
    fig.suptitle(
        ...
        color=TEXT, fontsize=9, fontfamily="monospace", fontweight="bold", y=0.98,
    )
    return _fig_to_pil(fig)
...
def training_chart(train_state) -> PIL.Image.Image:
    ...
    for i, (label, val) in enumerate(info):
        ...
        ax2.text(0.60, y, val, transform=ax2.transAxes,
                 color=ACC, fontsize=9, fontweight="bold", fontfamily="monospace")

    fig.suptitle("A2C TRAINING DASHBOARD", color=TEXT, fontsize=10,
                 fontfamily="monospace", fontweight="bold")
    fig.tight_layout()
    return _fig_to_pil(fig)
