...
def _make_mlp(in_dim: int, hidden: list[int], out_dim: int, activation: nn.Module) -> nn.Sequential:
...
class Actor(nn.Module):
    ...
    def __init__(self, obs_dim: int, act_dim: int, hidden: list[int] = [128, 128]):
        super().__init__()
...
class Critic(nn.Module):
    ...
    def __init__(self, obs_dim: int, hidden: list[int] = [128, 128]):
        super().__init__()
...
class PolicyNetwork(nn.Module):
    ...
    def __init__(self, obs_shape: int, n_actions: int):
        ...
        self.network = nn.Sequential(
            nn.Linear(obs_shape, 64), nn.ReLU(),
