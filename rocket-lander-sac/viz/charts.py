...
from core.mission import EpisodeResult, MissionResult
...
def mission_overview(mission: MissionResult) -> plt.Figure:
    ...
    colors = [
        GREEN if r >= 150 else (AMBER if r >= 0 else RED) for r in mission.rewards
    ]
    bars = ax1.bar(labels, mission.rewards, color=colors, edgecolor=BG, linewidth=0.8)
    ax1.axhline(
        200, color=GREEN, linestyle="--", linewidth=1, alpha=0.5, label="Perfect (200)"
    )
    ax1.axhline(
        150, color=ACCENT, linestyle="--", linewidth=1, alpha=0.5, label="Success (150)"
    )
    ...
    for i, ep in enumerate(mission.episodes):
        col = EP_COLORS[i % len(EP_COLORS)]
        ax3.plot(
            ep.cumulative_rewards,
            color=col,
            linewidth=1.5,
            label=f"#{ep.episode_idx + 1}",
            alpha=0.85,
        )
    ...
    ax4.fill_between(
        steps, 0, best.main_throttle, color=ACCENT, alpha=0.35, label="Main Engine"
    )
    ax4.plot(steps, best.main_throttle, color=ACCENT, linewidth=1.2)
    ax4.fill_between(
        steps,
        0,
        best.lateral_throttle,
        color=AMBER,
        alpha=0.25,
        label="Lateral Thrusters",
    )
...
def single_episode_detail(ep: EpisodeResult) -> plt.Figure:
    ...
    ax.scatter(
        ...
        s=80,
        marker="*" if ep.total_reward >= 150 else "x",
        color=GREEN if ep.total_reward >= 150 else RED,
        zorder=5,
        label="End",
    )
    ...
    ax.fill_between(
        steps, 0, cum, color=GREEN if ep.total_reward >= 150 else RED, alpha=0.2
    )
...
def training_dashboard(state) -> plt.Figure:
    ...
    gs = gridspec.GridSpec(
        1, 3, figure=fig, wspace=0.38, left=0.06, right=0.97, top=0.85, bottom=0.12
    )
