...
from bandits.agents import BanditAgent, ThompsonSampling, AGENT_COLORS
...
def _style(ax, title="", xlabel="", ylabel="", fontsize=9):
    ...
    if title:
        ax.set_title(title, color=TEXT, fontsize=fontsize, pad=8,
...
def comparison_dashboard(
    ...
    for name, agent in agents.items():
        ...
        ax1.plot(range(1, len(cum)+1), cum, color=_agent_color(name),
                 linewidth=2.0, label=name, alpha=0.9)
    # Optimal line
    opt = np.arange(1, steps+1) * optimal_ev
    ax1.plot(xs, opt, color=GRID, linewidth=1.2, linestyle="--",
             alpha=0.8, label="Optimal")
    ...
    for name, agent in agents.items():
        ...
        ax2.plot(range(1, len(rh)+1), rh, color=_agent_color(name),
                 linewidth=2.0, label=name, alpha=0.9)
    ...
    for i, (name, agent) in enumerate(agents.items()):
        ...
        bars   = ax3.bar(x_pos + offset, pcts, width=width * 0.9,
                         color=_agent_color(name), alpha=0.8,
                         edgecolor=BG, linewidth=0.5, label=name)
    ...
    y_base   = 0.95
    ax4.text(0.03, y_base, "Algorithm", transform=ax4.transAxes,
             color=DIM, fontsize=8, fontfamily="monospace")
    ax4.text(0.52, y_base, "Revenue", transform=ax4.transAxes,
             color=DIM, fontsize=8, fontfamily="monospace")
    ax4.text(0.76, y_base, "Regret", transform=ax4.transAxes,
             color=DIM, fontsize=8, fontfamily="monospace")
    ...
    for i, (name, rev, reg) in enumerate(zip(names, revenues, regrets)):
        ...
        ax4.text(0.03, y, f"● {name}{crown}", transform=ax4.transAxes,
                 color=col, fontsize=8, fontfamily="monospace", fontweight="bold")
        ax4.text(0.52, y, f"${rev:,.1f}", transform=ax4.transAxes,
                 color=UP if rev == best_rev else TEXT,
                 fontsize=8, fontfamily="monospace")
        ax4.text(0.76, y, f"${reg:,.1f}", transform=ax4.transAxes,
    ...
    fig.suptitle(
        ...
        color=TEXT, fontsize=10, fontfamily="monospace", fontweight="bold", y=0.97,
    )
    return fig
...
def belief_chart(
    ...
    if isinstance(agent, ThompsonSampling):
        ...
        for i, (ax, name, col) in enumerate(zip(axes, arm_names, arm_colors)):
            ...
            ax.axvline(true_ctrs[i], color=DOWN, linewidth=1.5,
                       linestyle="--", label=f"True: {true_ctrs[i]:.3f}")
            ax.axvline(a / (a + b), color=UP, linewidth=1.5,
                       linestyle=":", label=f"Est: {a/(a+b):.3f}")
            pulls = agent.counts[i]
            ax.text(0.97, 0.97, f"n={pulls}", transform=ax.transAxes,
                    ha="right", va="top", color=DIM, fontsize=7.5,
                    fontfamily="monospace")
            ax.legend(facecolor=BG3, edgecolor=GRID, labelcolor=TEXT, fontsize=7,
                      loc="upper left")

        fig.suptitle(f"THOMPSON SAMPLING — POSTERIOR BELIEFS  (Step {step})",
        ...
        fig.patch.set_facecolor(BG)
        _style(ax, f"{agent.name.upper()}  —  Q-VALUES  (Step {step})",
               "Banner", "Estimated Value ($)")

        x = np.arange(n)
        col_list = [arm_colors[i] for i in range(n)]
        bars = ax.bar(x, agent.values, color=col_list, edgecolor=BG,
                      linewidth=0.8, alpha=0.85)

        # True expected values overlay
        true_evs = [c * r for c, r in zip(true_ctrs,
        ...
        for i, (ctr, bar) in enumerate(zip(true_ctrs, bars)):
            ax.text(bar.get_x() + bar.get_width()/2,
                    ...
                    f"{agent.values[i]:.3f}\nn={agent.counts[i]}",
                    ha="center", color=TEXT, fontsize=7.5, fontfamily="monospace")
...
def campaign_analytics(
    ...
    col_widths = [0.28, 0.12, 0.16, 0.16, 0.18]
    y0 = 0.93
    ax1.text(0.02, y0, "  ".join(headers), transform=ax1.transAxes,
    ...
    for i, name in enumerate(arm_names):
        ...
        col  = arm_colors[i]
        is_best = true_ctrs[i] * revenues[i] == max(c * r for c, r in zip(true_ctrs, revenues))
        ax1.text(0.02, y, f"{'★ ' if is_best else '  '}{name[:14]}",
                 transform=ax1.transAxes, color=col,
                 fontsize=7.5, fontfamily="monospace")
        ax1.text(0.36, y, f"{pull:>5}", transform=ax1.transAxes,
                 color=TEXT, fontsize=7.5, fontfamily="monospace")
        ax1.text(0.50, y, f"{est:.3f}", transform=ax1.transAxes,
                 color=UP if abs(est - true_ctrs[i]) < 0.03 else TEXT,
                 fontsize=7.5, fontfamily="monospace")
        ax1.text(0.65, y, f"{true_ctrs[i]:.3f}", transform=ax1.transAxes,
                 color=DIM, fontsize=7.5, fontfamily="monospace")
        ax1.text(0.80, y, f"${rev:.1f}", transform=ax1.transAxes,
                 color=UP, fontsize=7.5, fontfamily="monospace")
    ...
    ax2.axhline(np.mean(rh) if len(rh) else 0, color=GOLD, linewidth=1,
    ...
    if wins > 0:
        ...
        for i in range(n):
            ax3.bar(range(wins), fracs[i], bottom=bottoms,
                    color=arm_colors[i], edgecolor=BG, linewidth=0,
                    alpha=0.85, label=arm_names[i])
            bottoms += fracs[i]
...
def scenario_preview(arm_names: list[str], true_ctrs: list[float],
                     revenues: list[float]) -> Figure:
    """Quick horizontal bar chart of expected value per banner."""
    n = len(arm_names)
    fig, ax = plt.subplots(figsize=(10, max(3, n * 0.7 + 1)), facecolor=BG)
    fig.patch.set_facecolor(BG)
    _style(ax, "BANNER EXPECTED VALUE  (CTR × Revenue)", "Expected Value ($/impression)")

    evs = [c * r for c, r in zip(true_ctrs, revenues)]
    best = max(evs)
    colors = plt.cm.Set2(np.linspace(0, 1, n))

    bars = ax.barh(arm_names, evs, color=colors, edgecolor=BG, linewidth=0.5, alpha=0.85)
    for bar, ev, name in zip(bars, evs, arm_names):
        ax.text(bar.get_width() + best * 0.01, bar.get_y() + bar.get_height()/2,
                f"${ev:.4f}", va="center", color=UP if ev == best else TEXT,
                fontsize=8.5, fontweight="bold" if ev == best else "normal")
...
def learner_step_chart(
    ...
    if isinstance(agent, ThompsonSampling):
        ...
        for i, name in enumerate(arm_names):
            ...
            ax1.plot(x, pdf, color=arm_colors[i], linewidth=2.0 if i == last_arm else 1.2,
                     alpha=alpha_v, label=name[:10])
            ax1.axvline(true_ctrs[i], color=arm_colors[i], linewidth=0.8,
                        linestyle=":", alpha=0.5)
        ax1.legend(facecolor=BG3, edgecolor=GRID, labelcolor=TEXT, fontsize=7)
    else:
        ...
        for i, v in enumerate(agent.values):
            ax1.text(i, v + max(agent.values)*0.01, f"{v:.3f}",
