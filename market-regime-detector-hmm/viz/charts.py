...
from core.data import REGIME_COLORS, DEFAULT_COLOR
...
def regime_price_chart(feat: pd.DataFrame, ticker: str) -> Figure:
    ...
    gs  = gridspec.GridSpec(3, 1, figure=fig, height_ratios=[4, 2, 0.4],
    ...
    for i, (d, lbl) in enumerate(zip(dates, labels)):
        if lbl != prev_label:
            if prev_label is not None:
                ax1.axvspan(band_start, d, alpha=0.12,
                            color=_color(prev_label), linewidth=0)
            band_start = d
            prev_label = lbl
    if prev_label:
        ax1.axvspan(band_start, dates[-1], alpha=0.12,
                    color=_color(prev_label), linewidth=0)
    ...
    ax1.legend(handles=handles, facecolor=BG3, edgecolor=GRID,
    ...
    ax1.set_title(
        ...
        color=TEXT, fontsize=10, pad=8, fontfamily="monospace", fontweight="bold",
    )
    ...
    for i in range(len(dates)):
        ax3.axvspan(dates[i], dates[i] if i == len(dates)-1 else dates[i+1],
                    facecolor=_color(labels.iloc[i]), alpha=0.9)
    ax3.set_yticks([])
...
def regime_analytics(
    ...
    if bic_scores:
        ...
        ax4.plot(ns, bics, color=UP, linewidth=2.2, marker="o", markersize=7,
                 markerfacecolor=BG2, markeredgecolor=UP, markeredgewidth=1.5)
        best_n = ns[np.argmin(bics)]
        ax4.axvline(best_n, color=NEUT, linewidth=1.2, linestyle="--",
...
def backtest_chart(bt: pd.DataFrame, metrics: dict, ticker: str) -> Figure:
    ...
    fig = plt.figure(figsize=(14, 9), facecolor=BG)
    gs  = gridspec.GridSpec(3, 1, figure=fig, height_ratios=[4, 2, 2],
                            hspace=0.45, left=0.08, right=0.97, top=0.91, bottom=0.07)
    ...
    ax1.plot(dates, bt["strategy_equity"], color=UP,   linewidth=2.2, label="Regime Strategy")
    ax1.plot(dates, bt["bnh_equity"],      color=NEUT, linewidth=1.8,
    ...
    ax2.plot(dates, bt["drawdown_bnh"],      color=NEUT, linewidth=1.0, linestyle="--", label="B&H")
    ax2.axhline(0, color=GRID, linewidth=0.8)
...
def multi_asset_chart(results: dict[str, pd.DataFrame]) -> Figure:
    ...
    fig, axes = plt.subplots(n + 1, 1, figsize=(14, 2 + n * 1.5),
                             facecolor=BG, gridspec_kw={"height_ratios": [3] * n + [0.3]})
    ...
    for ax, ticker in zip(axes[:n], tickers):
        ...
        for i in range(len(dates)):
            ...
            ax.axvspan(x0, x1, facecolor=_color(labels.iloc[i]), alpha=0.85, linewidth=0)

        ax.set_yticks([])
        ax.set_ylabel(ticker.upper(), color=TEXT, fontsize=9,
                      fontweight="bold", rotation=0, labelpad=36)
        for sp in ax.spines.values():
            sp.set_color(GRID)
    ...
    fig.legend(handles=handles, facecolor=BG3, edgecolor=GRID, labelcolor=TEXT,
               fontsize=8, loc="upper right", bbox_to_anchor=(0.97, 0.97))

    fig.suptitle("MULTI-ASSET REGIME SYNCHRONISATION",
...
def regime_gauge(probs: np.ndarray, label_map: dict) -> Figure:
    ...
    ax.set_title("CURRENT REGIME PROBABILITY", color=TEXT, fontsize=9,
                 pad=8, fontfamily="monospace", fontweight="bold")
    fig.tight_layout()
    return fig
